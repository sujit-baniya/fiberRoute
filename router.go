...
func (r *FiberGroup) Static(relativePath, root string) {
	r.getFiberRoutesWithMiddlewares().Static(pathToFiberPath(relativePath), root, fiber.Static{
		Compress:      true,
		ByteRange:     true,
...
func (r *FiberGroup) StaticFile(relativePath, filepath string) {
	r.getFiberRoutesWithMiddlewares().Static(pathToFiberPath(relativePath), filepath, fiber.Static{
		Compress:      true,
		ByteRange:     true,
		CacheDuration: 24 * time.Hour,
	})
...
func (r *FiberGroup) StaticFS(relativePath string, fs http.FileSystem) {
	panic("Not implemented yet on fiber")
}
